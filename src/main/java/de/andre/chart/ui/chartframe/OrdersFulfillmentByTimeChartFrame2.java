...
public class OrdersFulfillmentByTimeChartFrame2 extends JInternalFrameBase {
	 ...
	 *            the item to add
	 * @param creationTime
	 *            the creation time of that order (is the first time an
	 *            event for that item occurs)
	 */
	 ...
	 * @param commkey
	 *            an commkey
	 * @return current state or <code>null</code> if commkey (item) is
	 *         unknown
	 */
	...
	/**
	 * contains the current bar chart data. dimensions: (time, state) ->
	 * sum(quantity)
	 */
	...
	/**
	 * request to stop thread after the current task was finished. That can
	 * take a little while.
	 */
