...
public class OrdersFulfillmentByTimeChartFrame extends JInternalFrameBase {
    ...
    private final OrderItemEventGroups relevantEvents;
	...
	this.relevantEvents = new OrderItemEventGroups(dateTimeLookup);
	...
	if (repaintChartThread == null || repaintChartThread.isInterrupted() || !repaintChartThread.isAlive()) {
	...
	for (LocalDateTime timeGroup : events.getAllSortedKeys()) {
	    ...
	    final HashMap<Integer, OrderItemEvent> relevantEvent = new HashMap<>();
		...
		if (time == null || event.newState() == OrderItemState.PROCESSED.getId()
		    ...
		    relevantEvent.put(commkey, event);
		}
	    ...
	    for (OrderItemEvent event: relevantEvent.values()) {
	...
	for (LocalDateTime timeGroup : relevantEvents.getAllSortedKeys()) {
	    for (OrderItemEvent event : relevantEvents.getSortedEvents(timeGroup, eventComparator)) {
		final int commkey = event.commkey();
